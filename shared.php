...
<body>
        ...
        <!-- <div class="row projects gx-lg-5"> -->
            <!-- <a href="work-single.html" class="col-sm-6 col-lg-4 text-decoration-none project marketing social business"> -->
                <div class="service-work overflow-hidden card mb-5 mx-5 m-sm-0">
                    ...
                    <h5 class="h5 text-center col-12 py-5 semi-bold-600">Shared files</h5> 
        <form name="f" method="post" enctype="multipart/form-data" action="">
            <input type="text"  placeholder="filename" class="form-control form-control-lg rounded-pill rounded-end" name="filename"/><br/>
            <input type="textarea"  placeholder="Description" class="form-control form-control-lg rounded-pill rounded-end" name="description"/><br/>
            <select name="send" placeholder="send to" class="form-control form-control-lg rounded-pill rounded-end" >
            <option>receiver</option>
                <?php
            include './dbconnection.php';
            $query="select * from tbluser where allowed='y' and not username='{$_SESSION['id']}'";
            ...
            if(isset($_GET['r'])){
                $query="select * from tbluser where username='{$_GET['u']}'";
                $result=mysqli_query($link,$query);
            ...
            <input type="file"  placeholder="File" class="form-control" name="myfile"/><br/>
            <input type="submit" class="btn btn-secondary text-white btn-lg rounded-pill rounded-start px-lg-4" value="upload" name="sub"/>
        </form> 
            </div>
            <?php
        if(isset($_REQUEST['sub'])){
            $link=mysqli_connect("localhost","root","","filedistribution");
            $filename=$_POST['filename'];
            $description=$_POST['description'];
            $name=$_FILES['myfile']['name'];
            $query="insert into sharedfile(filename,description,uploadedfile,receiver,username)values('$filename','$description','$name','{$_POST['send']}','{$_SESSION['id']}')";
            $result=mysqli_query($link,$query);
            if($result){
    ...
    <table class="table" border="2px;">
            ...
            <?php
            include './dbconnection.php';
            $query="select * from sharedfile where username='{$_SESSION['id']}'";
            $result=mysqli_query($link,$query);
            while($row=mysqli_fetch_assoc($result)){
            ?>
            <tr>
